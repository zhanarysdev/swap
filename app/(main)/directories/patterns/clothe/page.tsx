...
export default function ClothePage() {
  ...
  const { data, isLoading, mutate } = useSWR(
    "http://localhost:4000/clothe",
    fetcher
  );
  const remove = async () => {
    const res = await fetch(`http://localhost:4000/clothe/${isDelete}`, {
      method: "DELETE",
    });
  ...
  const save = async () => {
    ...
    if (isEdit) {
      const res = await fetch(`http://localhost:4000/clothe/${isEdit}`, {
        method: "PUT",
      ...
      const res = await fetch("http://localhost:4000/clothe", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: value,
